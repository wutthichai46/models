...
python -m intel_extension_for_pytorch.cpu.launch --socket_id 0  --enable_jemalloc --log_path=${OUTPUT_DIR} --log_file_prefix="./throughput_log_phase2_${precision}" ${TRAIN_SCRIPT} \
    --input_dir ${DATASET_DIR}/2048_shards_uncompressed_512/ \
    --eval_dir ${DATASET_DIR}/eval_set_uncompressed/ \
>>>>>>> 8fb9dc117... Enable jemalloc for BERT throughput mode (#375)
    --model_type 'bert' \
